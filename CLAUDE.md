...
1. **【情報整理】NotebookLM** — 関連ドキュメントを読み込ませ、要件定義書・実装ステップ（Claude への指示書）を作成する
2. **【実装】Claude Code** — NotebookLM が作成した詳細な要件・指示書を受け取ってから実装を開始する

**Claude へのお願い：**
- 要件・指示書が渡されていない段階では、実装に入る前に「NotebookLMで要件を整理しましたか？」と確認すること
- 曖昧な指示でいきなりコードを書き始めず、要件の明確化を優先すること
- 1セッションのスコープを指示書に書かれた範囲に限定し、範囲外の改善を勝手に行わないこと
